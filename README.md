...
> Read [`docs/portfolio-improvement-status.md`](docs/portfolio-improvement-status.md)
> and [`docs/operations/security-rollout.md`](docs/operations/security-rollout.md)
> first. They separate implemented code from deployment-gated work.

**Tech Stack:**
- **Framework:** React 18.2.0 with TypeScript 5.8.2
- **Build Tool:** Vite 6.2.0
- **Router:** React Router DOM 6.23.0 (BrowserRouter with legacy hash redirect)
- **Animations:** Framer Motion 10.18.0
- **3D Graphics:** Three.js 0.160.0 with React Three Fiber & Drei
- **Icons:** Lucide React 0.344.0
- **AI Integration:** server-only Google Gemini provider exposed through `/api/ai/chat`
...
│   ├── ProjectDetail.tsx   # Shareable project case study
│   └── Work.tsx            # Projects showcase
├── context/            # React Context providers
│   ├── GamificationContext.tsx  # Achievements & easter eggs
│   ├── MotionContext.tsx        # Reduced/pause motion policy
│   └── ThemeContext.tsx         # Theme state management
├── services/api/       # Typed browser-to-server API adapters
├── server/             # Server-only providers, auth and repositories
...
### 2. **React Router with BrowserRouter**
Uses semantic paths with a pre-router compatibility redirect for legacy hashes:
```tsx
...
Vercel and Cloudflare both use an SPA fallback; legacy `#/...` URLs are replaced without reload.
...
|-------------------|----------|-----------------------------------------------------------------------|
| `ADMIN_SESSION_SECRET` | Yes | HMAC secret for fixed-TTL stateful session cookies.                    |
| `ADMIN_PASSWORD_HASH`  | Yes | Scrypt hash generated by `npm run admin:hash-password -- "..."`.       |
...
Configure independent session/rate-limit secrets and the scrypt password hash.
Visit `/admin/login`; the server stores a hash of the session ID and sends an
HttpOnly, SameSite=Strict cookie. Unsafe admin requests require CSRF.

**Admin routes** (semantic paths, e.g. `https://your-site.com/admin`):

- `/admin/login` — password login
- `/admin` — dashboard with stats and quick links
- `/admin/projects`, `/admin/tools`, `/admin/skills`, `/admin/milestones`
- `/admin/experiments`, `/admin/messages`
...
matches them internally. `api-server.ts` continues to host the same
...
### Service boundary
- Browser: `services/api/ai.ts` calls `POST /api/ai/chat`
- Server: `server/ai.ts` invokes the Gemini provider and atomic DB rate limit
...
- **Error Handling:** provider quota maps to `503 AI_UNAVAILABLE`
...
- **Server-only cooldown state** for the lifetime of the function instance
...
| `server/providers/gemini.ts` | Server-only Gemini persona and response handling | Modifying AI provider behavior |
| `server/providers/gemini-key-manager.ts` | Server-only key rotation and cooldown | Changing provider key handling |
...
| `react-router-dom` | 6.30.4 | Client-side routing (BrowserRouter) |
...
- BrowserRouter with tested legacy hash redirect and SPA fallbacks
